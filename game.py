...
class PacmanGame(arcade.View):
    ...
    def setup(self):
        ...
        for row_idx, row in enumerate(LEVEL_MAP):
            for col_idx, cell in enumerate(row):
                ...
                y = (len(LEVEL_MAP) - row_idx - 1) * TILE_SIZE + TILE_SIZE / 2 + self.offset_y
        ...
        self.map_right = self.offset_x + map_width
        self.map_bottom = self.offset_y
        self.map_top = self.offset_y + map_height
    ...
    def on_draw(self):
        ...
        if self.game_over:
            ...
            arcade.draw_rect_filled(arcade.rect.XYWH(cx, cy, TILE_SIZE, TILE_SIZE / 2), arcade.color.GRAY)
            arcade.draw_text("RESTART", cx, cy, arcade.color.GREEN, 25, anchor_x="center", anchor_y="center")
    ...
    def on_update(self, delta_time):
        ...
        for apple in hit_apples:
            ...
            self.player.power_mode, self.player.power_time = True, 5.0

        if self.player.power_mode:
            self.player.power_time -= delta_time
            if self.player.power_time <= 0: self.player.power_mode = False

        ghost_hit_list = arcade.check_for_collision_with_list(self.player, self.ghost_list)
        if ghost_hit_list:
            if self.player.power_mode:
                for ghost in ghost_hit_list:
                    ghost.center_x, ghost.center_y = self.get_random_free_position()
                    self.player.score += 10
            else:
                self.player.lives -= 1
                if len(self.heart_list) > 0: self.heart_list.pop()
